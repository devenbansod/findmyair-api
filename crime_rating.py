...
def compute_pairwise_distance(airbnb_dataset, crime_dataset):
    ...
    if path.exists("data/distance_matrix.npy"):
        print ("Cached distance matrix found")
        return np.load("data/distance_matrix.npy",mmap_mode='r+')
...
def get_crime_score(crime_weight, distance_matrix):

    num_airbnb = distance_matrix.shape[0]
    crime_score = np.zeros(num_airbnb)

    if path.exists("data/crime_scores.npy"):
        print ("Cached crime scores found")
        return np.load("crime_score.npy")
    ...
    np.save("crime_scores", crime_score)
    return crime_score
...
if __name__ == "__main__":
    ...
    #Calculate seriousness of each crime
    crime_weight = set_weight(crime_dataset)

    #Compute and cache pairwise distance matrix
    distance_matrix = compute_pairwise_distance(airbnb_dataset, crime_dataset)

    #Compute crime score for each Airbnb
    # crime_score = get_crime_score(crime_weight, distance_matrix)
